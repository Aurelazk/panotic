...
router.get('/carte/villes', (req, res) => {
  res.json(villes);
});

router.get('/carte/signalements', (req, res) => {
  const { type, villeId } = req.query;
  let filtered = filterByVille(signalements, villeId);
  if (type && type !== 'tous') {
...
router.get('/carte/panneaux', (req, res) => {
  const { type, etat, villeId } = req.query;
  let filtered = filterByVille(panneaux, villeId);
  if (type && type !== 'tous') {
...
router.get('/carte/zones', (req, res) => {
  const { villeId } = req.query;
  res.json(filterByVille(zones, villeId));
});

router.get('/carte/recherche', (req, res) => {
  const { q, villeId } = req.query;
  if (!q) return res.json({ panneaux: [], zones: [] });

  const query = q.toLowerCase();
  let filteredPanneaux = filterByVille(panneaux, villeId);
  let filteredZones = filterByVille(zones, villeId);
...
router.get('/carte/heatmap', (req, res) => {
  const { villeId } = req.query;
  const data = filterByVille(signalements, villeId)
...
router.get('/carte/stats', (req, res) => {
  const { villeId } = req.query;
  const ville = villes.find(v => v.id === villeId) || { nom: 'Toutes les villes', lat: 6.5, lng: 2.0 };

  const sigs = filterByVille(signalements, villeId);
  const pans = filterByVille(panneaux, villeId);
  const zns = filterByVille(zones, villeId);
